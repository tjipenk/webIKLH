...
<div class="pull-right hidden-xl hidden-lg" style="margin-top: 20px">
				<a href="<?php echo site_url('admin/add_data_udara'); ?>" id="tambah"style="background: #80b500;color: #fff;padding: 15px 20px 15px 0px;margin:20px 0 0 0;border-radius:30px;position:fixed;bottom:20px;right:30px"><i class="glyph-icon flaticon-add"></i></a>
				</div>
...
$("#usersform").on('submit',(function(e) {
        e.preventDefault();
                ...
                $.post("<?php echo base_url(); ?>admin/load_data_udara/<?php echo $tahun;?>", {
                p:p,
...
function reject(id){
		...
		if (result) {
			...
			$.post("<?php echo base_url(); ?>admin/removeDataUdara", {
			i:id,
			<?php echo $this->security->get_csrf_token_name(); ?>: '<?php echo $this->security->get_csrf_hash(); ?>'				
			},
			function(data){
...
function validate(id){
		...
		if (result) {
			...
			$.post("<?php echo base_url(); ?>admin/validateDataUdara", {
			i:id,
			<?php echo $this->security->get_csrf_token_name(); ?>: '<?php echo $this->security->get_csrf_hash(); ?>'				
			},
			function(data){
