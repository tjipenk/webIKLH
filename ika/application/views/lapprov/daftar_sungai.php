<div class="container header">
	...
	<a href="<?php echo site_url('lap_prov/add_sungai'); ?>" style="float:right;">
		<br />
...
<div class="pull-right hidden-xl hidden-lg" style="margin-top: 20px">
	<a href="<?php echo site_url('lap_prov/add_sungai'); ?>" id="tambah"style="background: #80b500;color: #fff;padding: 15px 20px 15px 0px;margin:20px 0 0 0;border-radius:30px;position:fixed;bottom:20px;right:30px"><i class="glyph-icon flaticon-add"></i></a>
</div>
...
<script type='text/javascript'>
	jQuery(document).ready(function($){
		$("#usersform").on('submit',(function(e) {
				e.preventDefault();
						...
						$.post("<?php echo base_url(); ?>lap_prov/load_sungai", {
						p:p,
						<?php echo $this->security->get_csrf_token_name(); ?>: '<?php echo $this->security->get_csrf_hash(); ?>'				
						},
						function(data){
							$("#usersarea").html(data);
							$('#datatable').DataTable(); // ini yang buat datatables nya ya   <<<--------
						});
				return false;		
		}));
	...
	function delete_sungai(id){
		...
		if (result) {
			...
			$.post("<?php echo base_url(); ?>lap_prov/removesungai", {
			i:id,
			<?php echo $this->security->get_csrf_token_name(); ?>: '<?php echo $this->security->get_csrf_hash(); ?>'				
			},
			function(data){
